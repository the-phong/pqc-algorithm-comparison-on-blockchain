"""End-to-end verifier for Dilithium records stored on-chain."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify a Dilithium record end-to-end.")
    parser.add_argument("--record-id", type=int, required=True, help="On-chain record id to verify")
    parser.add_argument(
        "--public-key-cid",
        required=True,
        help="CID of the stored Dilithium public key used to verify the record",
    )
...
def main() -> None:
    ...
    signature_valid = bool(ml_dsa_44.verify(public_key, record["payload"], signature))
    ...
    print("Dilithium End-to-End Verification")
