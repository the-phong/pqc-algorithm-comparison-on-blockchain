"""End-to-end verifier for Falcon records stored on-chain."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify a Falcon record end-to-end.")
    parser.add_argument("--record-id", type=int, required=True, help="On-chain record id to verify")
    parser.add_argument(
        "--public-key-cid",
        required=True,
        help="CID of the stored Falcon public key used to verify the record",
    )
...
def main() -> None:
    ...
    signature_valid = bool(falcon_512.verify(public_key, record["payload"], signature))
    ...
    print("Falcon End-to-End Verification")
